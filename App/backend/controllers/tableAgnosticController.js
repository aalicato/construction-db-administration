...
const getDBInfo = async (req, res) => {
  try {
    const rows =
      await db.query(`SELECT TABLE_NAME, COLUMN_NAME, COLUMN_COMMENT, IS_NULLABLE, COLUMN_KEY FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE();
        SELECT 
...
const getRecordsByColumn = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Error fetching records for this table and column` });
  }
};
